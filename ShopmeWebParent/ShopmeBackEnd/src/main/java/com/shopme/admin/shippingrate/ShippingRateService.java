...
import com.shopme.admin.setting.city.CityRepository;
...
public class ShippingRateService {
	...
	public List<String> listStatesByCountryStr(Country country) {

		List<State> states = stateRepository.findByCountryOrderByNameAsc(country);
		List<String> statesStr = new ArrayList<>();
		for (State state : states) {
			statesStr.add(state.getName());
		}
		return statesStr;
	}

	public List<State> listStatesByCountry(Country country) {

		return stateRepository.findByCountryOrderByNameAsc(country);
	}

	public List<City> listCitiesByStateName(String stateStr) {
	...
	public void save(ShippingRate rateInForm) throws ShippingRateAlreadyExistsException {
		ShippingRate rateInDB = shipRepo.findByCountryAndStateAndCity(rateInForm.getCountry().getId(), rateInForm.getState(),rateInForm.getCity());
				
				//shipRepo.findByCountryAndState(rateInForm.getCountry().getId(), rateInForm.getState());
		...
		if (foundExistingRateInNewMode || foundDifferentExistingRateInEditMode) {
			throw new ShippingRateAlreadyExistsException("There's already a rate for the destination "
					+ rateInForm.getCountry().getName() + ", " + rateInForm.getState() + ", "+ rateInForm.getCity());
		}
	...
	public float calculateShippingCost(Integer productId, Integer countryId, String state, String city)
			throws ShippingRateNotFoundException {
		ShippingRate shippingRate = shipRepo.findByCountryAndStateAndCity(countryId, state,city);
				//shipRepo.findByCountryAndState(countryId, state);
