...
public interface ShippingRateRepository extends SearchRepository<ShippingRate, Integer> {
	...
	@Query("SELECT sr FROM ShippingRate sr WHERE sr.country.id = ?1 AND sr.state = ?2 AND sr.city = ?3")
	public ShippingRate findByCountryAndStateAndCity(Integer countryId, String state,String city);
	
	@Query("UPDATE ShippingRate sr SET sr.codSupported = ?2 WHERE sr.id = ?1")
	@Modifying
	public void updateCODSupport(Integer id, boolean enabled);
	
	@Query("SELECT sr FROM ShippingRate sr WHERE sr.country.name LIKE %?1% OR sr.state LIKE %?1% OR sr.city LIKE %?1%")
